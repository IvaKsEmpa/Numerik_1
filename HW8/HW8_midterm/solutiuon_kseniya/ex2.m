...
figure
loglog(N, t1,'--*', N, N.^3/N(1)^3*t1(1)*0.7,'LineWidth', 2) % /N(1)^3*t1(1)*0.7
xlabel('dimension N')
...
for k = 1:p
    ...
    x= linsolve(A,b);
    x1= linsolve(A1,b);
    err(k)=norm(x-xdelta);%max(abs(x-xdelta));            % compute error
    err1(k)=norm(x1-xdelta1);%max(abs(x1-xdelta1));            % compute error
